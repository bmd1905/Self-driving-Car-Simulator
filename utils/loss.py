...
class SigmoidBin(nn.Module):
    ...
    def __init__(self, bin_count=10, min=0.0, max=1.0, reg_scale=2.0, use_loss_regression=True, use_fw_regression=True, BCE_weight=1.0, smooth_eps=0.0):
        ...
        self.BCEbins = nn.BCEWithLogitsLoss(
    ...
    def forward(self, pred):
        ...
        pred_reg = (pred[..., 0] * self.reg_scale -
                    self.reg_scale/2.0) * self.step
    ...
    def training_loss(self, pred, target):
        ...
        pred_reg = (pred[..., 0].sigmoid() * self.reg_scale -
                    self.reg_scale/2.0) * self.step
        pred_bin = pred[..., 1:(1+self.bin_count)]
        ...
        target_bins = torch.full_like(
            pred_bin, self.cn, device=device)  # targets
        n = pred.shape[0]
        ...
        for ii in order:
            ...
            if delta_RS > 0:
                fg_relations = torch.clamp(
                    fg_relations/(2*delta_RS)+0.5, min=0, max=1)
                bg_relations = torch.clamp(
                    bg_relations/(2*delta_RS)+0.5, min=0, max=1)
            else:
            ...
            current_sorting_error = torch.sum(
                fg_relations*(1-fg_targets))/rank_pos
            ...
            target_sorting_error = torch.sum(
                target_sorted_order*(1-fg_targets))/rank_pos_target
            ...
            if sorting_pmf_denom > eps:
                ...
                fg_grad += (missorted_examples *
                            (sorting_error[ii]/sorting_pmf_denom))
...
class aLRPLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, regression_losses, delta=1., eps=1e-5):
        ...
        for ii in order:
            ...
            fg_relations = torch.clamp(
                fg_relations/(2*delta)+0.5, min=0, max=1)
            ...
            bg_relations = torch.clamp(
                bg_relations/(2*delta)+0.5, min=0, max=1)
            ...
            if FP_num > eps:
                fg_grad[ii] = -(torch.sum(fg_relations *
                                regression_losses)+FP_num)/rank[ii]
    ...
    def forward(ctx, logits, targets, delta=1.):
        ...
        for ii in order:
            ...
            if (max_prec <= current_prec):
                ...
                relevant_bg_grad += (bg_relations /
                                     rank[ii])*(((1-max_prec)/(1-current_prec)))
...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        bs, as_, gjs, gis, targets, anchors = self.build_targets(
            p, targets, imgs)
        pre_gen_gains = [torch.tensor(pp.shape, device=device)[
            [3, 2, 3, 2]] for pp in p]
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou(prediction, target)
                iou = bbox_iou(pbox.T, selected_tbox,
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                from_which_layer.append(
                    (torch.ones(size=(len(b),)) * i).to(device))
                ...
                grid = torch.stack([gi, gj], dim=1)
                # / 8.
                pxy = (fg_pred[:, :2].sigmoid() * 2. -
                       0.5 + grid) * self.stride[i]
                # pxy = (fg_pred[:, :2].sigmoid() * 3. - 1. + grid) * self.stride[i]
                # / 8.
                pwh = (fg_pred[:, 2:4].sigmoid() * 2) ** 2 * \
                    anch[i][idx] * self.stride[i]
            ...
            top_k, _ = torch.topk(pair_wise_iou, min(
                10, pair_wise_iou.shape[1]), dim=1)
...
class ComputeLossBinOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        wh_bin_sigmoid = SigmoidBin(
            bin_count=self.bin_count, min=0.0, max=4.0, use_loss_regression=False).to(device)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                w_loss, pw = self.wh_bin_sigmoid.training_loss(
                    ps[..., 2:(3+self.bin_count)], selected_tbox[..., 2] / anchors[i][..., 0])
                h_loss, ph = self.wh_bin_sigmoid.training_loss(
                    ps[..., (3+self.bin_count):obj_idx], selected_tbox[..., 3] / anchors[i][..., 1])
                ...
                pbox = torch.cat((px.unsqueeze(1), py.unsqueeze(1), pw.unsqueeze(
                    1), ph.unsqueeze(1)), 1).to(device)  # predicted box
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(
                        ps[:, (1+obj_idx):], self.cn, device=device)  # targets
                    t[range(n), selected_tcls] = self.cp
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                ...
                pw = self.wh_bin_sigmoid.forward(fg_pred[..., 2:(
                    3+self.bin_count)].sigmoid()) * anch[i][idx][:, 0] * self.stride[i]
                ph = self.wh_bin_sigmoid.forward(
                    fg_pred[..., (3+self.bin_count):obj_idx].sigmoid()) * anch[i][idx][:, 1] * self.stride[i]

                pxywh = torch.cat(
                    [pxy, pw.unsqueeze(1), ph.unsqueeze(1)], dim=-1)
                pxyxy = xywh2xyxy(pxywh)
                pxyxys.append(pxyxy)
...
class ComputeLossAuxOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        bs, as_, gjs, gis, targets, anchors = self.build_targets(
            p[:self.nl], targets, imgs)
        pre_gen_gains_aux = [torch.tensor(pp.shape, device=device)[
            [3, 2, 3, 2]] for pp in p[:self.nl]]
        pre_gen_gains = [torch.tensor(pp.shape, device=device)[
            [3, 2, 3, 2]] for pp in p[:self.nl]]
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            # image, anchor, gridy, gridx
            b, a, gj, gi = bs[i], as_[i], gjs[i], gis[i]
            b_aux, a_aux, gj_aux, gi_aux = bs_aux[i], as_aux_[
                i], gjs_aux[i], gis_aux[i]  # image, anchor, gridy, gridx
            tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj
            tobj_aux = torch.zeros_like(
                pi_aux[..., 0], device=device)  # target obj
            ...
            if n_aux:
                ...
                tobj_aux[b_aux, a_aux, gj_aux, gi_aux] = (
                    1.0 - self.gr) + self.gr * iou_aux.detach().clamp(0).type(tobj_aux.dtype)  # iou ratio

                # Classification
                selected_tcls_aux = targets_aux[i][:, 1].long()
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t_aux = torch.full_like(
                        ps_aux[:, 5:], self.cn, device=device)  # targets
            ...
            lobj += obji * self.balance[i] + 0.25 * \
                obji_aux * self.balance[i]  # obj loss
            if self.autobalance:
                self.balance[i] = self.balance[i] * \
                    0.9999 + 0.0001 / obji.detach().item()
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            top_k, _ = torch.topk(pair_wise_iou, min(
                20, pair_wise_iou.shape[1]), dim=1)
            dynamic_ks = torch.clamp(top_k.sum(1).int(), min=1)
